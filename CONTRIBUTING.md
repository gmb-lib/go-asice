...
- **Fuzz** — every `Fuzz*` target runs for 30 seconds. This module has fuzz targets (`FuzzSniff`, `FuzzInspect`) because it reads containers from bytes nobody trusts; a change to container reading should extend them rather than work around them.
...
This library makes exactly one integrity claim — that the signatures in a container reference
exactly the documents beside them, by name and by digest. Any change that touches reference
checking, digest comparison, entry naming or the strict outer-shape check is a change to that
claim, and needs a test that fails without it.

Two rules are load-bearing rather than stylistic:

- **No cryptographic verification belongs here.** Signature validity is a validator's answer, not
  this library's. A pull request that starts verifying signatures is a design change, not a fix —
  raise it as an issue first.
- **No network, no filesystem, no HTTP.** The dependency surface is the standard library, and it
  stays that way: XML reading must not resolve external entities, and container reading must keep
  its size, entry-count and decompression limits.
